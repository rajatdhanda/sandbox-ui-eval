// app/components/admin/class-management.tsx
// Path: app/components/admin/class-management.tsx
...
export const ClassManagement = () => {
  ...
  console.log('🎯 [CLASS_MANAGEMENT] Component initializing...');
  ...
  const handleSubmit = async () => {
    try {
      ...
      // Clean data before submission - remove any fields not in the database
      const submitData: any = {
        first_name: crud.formData.first_name,
        last_name: crud.formData.last_name,
        date_of_birth: crud.formData.date_of_birth,
        class_id: crud.formData.class_id || null,
        medical_notes: crud.formData.medical_notes || null,
        allergies: crud.formData.allergies || null,
        emergency_contact: crud.formData.emergency_contact || null,
        emergency_phone: crud.formData.emergency_phone || null,
        enrollment_date: crud.formData.enrollment_date,
        is_active: crud.formData.is_active
      };
      
      // If editing, include the ID
      if (crud.editingItem) {
        submitData.id = crud.editingItem.id;
      }
      
      console.log('📤 Cleaned submit data:', submitData);
      
      // Override the formData temporarily for submission
      const originalFormData = crud.formData;
      crud.setFormData(submitData);
      
      await crud.handleSubmit();
      
      // Restore original form data structure
      crud.setFormData(originalFormData);
      ...
      if (error?.message) {
        ...
        } else if (error.message.includes("Could not find the 'attachments'")) {
